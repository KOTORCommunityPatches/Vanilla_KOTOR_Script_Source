 ...
 void main() {
 	...
 	if (nUser == 1001) // HEARTBEAT
 			...
 			if (GetCurrentHitPoints(OBJECT_SELF) <= nHP && GetGlobalNumber("k_Rulan_morph") == 2)
 				{
 					...
 					AssignCommand(GetModule(), DelayCommand(0.4, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(1046), GetLocation(OBJECT_SELF))));
 	...
 	else if (nUser == 1007) // DEATH
 		{
 			...
 			AddJournalQuestEntry("Geno_Rulan", 50);
 			UT_DarkSml(GetFirstPC());
 			ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(1047), lSelf);
 			ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_IMP_SCREEN_SHAKE), GetFirstPC());
 			DelayCommand(0.6, CreateNPC(OBJECT_TYPE_PLACEABLE, "rulan_corpse", lSelf));
