		...
		if ((int1 == 1002)) {
			if (GetLastPerceptionSeen()) {
				if ((GetLastPerceived() == GetPartyMemberByIndex(0))) {
					sub1("Player is Seen", 5, 5, 4.0);
					ClearAllActions();
					if (((GetTag(OBJECT_SELF) == "CaloNord") && (GetGlobalNumber("K_KALO_BANDON") >= 10))) {
						sub1("Talking To Play", 5, 6, 4.0);
						sub2(intGLOB_74, 0);
						ActionStartConversation(GetPartyMemberByIndex(0), "k_h_calo", 0, 0, 1, "", "", "", "", "", "");
					}
					else {
						if (((GetTag(OBJECT_SELF) == "g_bandon") && (GetGlobalNumber("K_KALO_BANDON") >= 30))) {
							sub2(intGLOB_74, 0);
							ActionStartConversation(GetPartyMemberByIndex(0), "k_h_bandon", 0, 0, 1, "", "", "", "", "", "");
						}
					}
				}
			}
