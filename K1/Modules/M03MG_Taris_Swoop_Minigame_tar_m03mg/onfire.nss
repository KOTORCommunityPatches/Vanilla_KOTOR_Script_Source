...
int StartingConditional() {
	...
	float float4 = 150.0;
	float float5 = 210.0;
	float float6 = (GetGlobalNumber("TAR_SWOOP_ACCEL") / 10.0);
	if ((GetGlobalNumber("TAR_SWOOP_ACCEL") <= 0)) {
		float6 = 1.0;
	}
	float float7 = (20.0 * float6);
	float float8 = (8.0 * float6);
	float float9 = (6.0 * float6);
	float float10 = (3.0 * float6);
	float float11 = (2.0 * float6);
	object oEngine05;
	int nGlobal = GetGlobalNumber("MIN_RACE_GEAR");
	float float12 = SWMG_GetPlayerSpeed();
	int int5 = nGlobal;
	if ((nGlobal == (-1))) {
					...
					if ((nGlobal == 3)) {
						...
						if ((nGlobal == 4)) {
							if ((float12 > float5)) {
								...
								SWMG_SetPlayerAccelerationPerSecond(float11);
							}
						}
					}
				}
