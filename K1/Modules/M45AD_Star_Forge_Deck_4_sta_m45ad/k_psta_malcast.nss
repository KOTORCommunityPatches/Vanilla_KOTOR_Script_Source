...
void main() {
	...
	object oPCWP = GetNearestObjectByTag("sta_pc_cutscene_jump", oMalak, 1);
	...
	if (GetDistanceBetween(oPC, oMalak) < 3.0)
		{
			AssignCommand(oPC, ClearAllActions());
			DelayCommand(0.1, AssignCommand(oPC, JumpToLocation(lPCWP)));
		}
	
	SoundObjectStop(oBuzz);
	
	AssignCommand(oMalak, ClearAllActions());
	AssignCommand(oMalak, JumpToLocation(lFace));
	...
	DelayCommand(3.2, ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_IMP_SCREEN_SHAKE, FALSE), GetFirstPC(), 0.0));
	
	DelayCommand(2.7, DrainCaptive());
	DelayCommand(2.7, AssignCommand(oCaptive, PlayAnimation(ANIMATION_PLACEABLE_DEACTIVATE, 1.0, 0.0)));
