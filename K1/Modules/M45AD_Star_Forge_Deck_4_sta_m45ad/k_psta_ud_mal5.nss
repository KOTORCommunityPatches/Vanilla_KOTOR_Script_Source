	...
	if (nUser == 1001) // HEARTBEAT
		{
			...
			if (nCurFP <= 50 && GetIsObjectValid(oDrain) && (GetCurrentHitPoints(oPC) > (GetMaxHitPoints(oPC) / 2)))
				{
				...
				else if (nTalk > 0 && !GetIsInConversation(oInvis1) && !GetIsInConversation(OBJECT_SELF) && nDrain != TRUE)
					{
