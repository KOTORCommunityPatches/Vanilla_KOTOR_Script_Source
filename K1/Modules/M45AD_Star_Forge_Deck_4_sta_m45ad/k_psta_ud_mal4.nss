...
void main() {
	...
	if (nUser == 1001) // HEARTBEAT
	...
	else if (nUser == 1006) // DAMAGED
		{
			...
			object oPC = GetFirstPC();
			object oInvis1 = GetObjectByTag("sta_invis_conv_2", 0);
					...
					if (GetIsObjectValid(oDrain))
						{
							...
							if (GetDistanceToObject(oDrain) <= 5.0)
								{
								...
								else
									{
										...
										DelayCommand(1.0, ActionMoveToObject(oDrain, 1, 1.0));
						...
						else if (nCurHP <= 10 && !GetIsInConversation(OBJECT_SELF))
							{
								...
								DelayCommand(3.5, AssignCommand(oInvis1, ActionStartConversation(oInvis1, "", FALSE, CONVERSATION_TYPE_CINEMATIC, FALSE)));
							}
				}
		}
