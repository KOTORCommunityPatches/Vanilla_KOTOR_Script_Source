...
void main() {
	...
	object oSword = GetItemInSlot(INVENTORY_SLOT_RIGHTWEAPON, oGuard);
	location lLand = GetLocation(GetObjectByTag("tar_bastila_force", 0));
	location lSword = GetLocation(GetObjectByTag("tar03_wpsword", 0));
	string sSword = "g_w_vbrdblswd01";
	...
	DelayCommand(1.1, AssignCommand(GetObjectByTag("tar03_bastcage", 0), PlayAnimation(ANIMATION_PLACEABLE_OPEN)));
	...
	DelayCommand(3.0, ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDeath(), oGuard));
