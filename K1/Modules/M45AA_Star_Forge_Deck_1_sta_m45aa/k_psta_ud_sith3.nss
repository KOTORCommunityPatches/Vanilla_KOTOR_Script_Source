			...
			if (GetGlobalNumber("STA_JEDI_ATTACK") == 6)
				{
					...
					object oJedi3 = GetObjectByTag("k_sta_lightjedi3", 0);
					talent talSpell = TalentSpell(FORCE_POWER_LIGHTNING);
					
					ActionUseTalentOnObject(talSpell, oJedi3);
					
					event eUser = EventUserDefined(452);
					
					DelayCommand(5.0, SignalEvent(oJedi3, eUser));
