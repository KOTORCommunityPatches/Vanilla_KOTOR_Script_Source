...
void main() {
	...
	if (GetGlobalBoolean("UNK_TILES") == FALSE)
		{
			if (UT_GetTalkedToBooleanFlag(oFloor103) == TRUE)
				{
					AssignCommand(oFloor103, ActionPlayAnimation(ANIMATION_PLACEABLE_ACTIVATE));
					UT_SetTalkedToBooleanFlag(oFloor103, FALSE);
				}
				else
					{
						AssignCommand(oFloor103, ActionPlayAnimation(ANIMATION_PLACEABLE_DEACTIVATE));
						UT_SetTalkedToBooleanFlag(oFloor103, TRUE);
					}
			...
			if (UT_GetTalkedToBooleanFlag(oFloor106) == TRUE)
				{
					AssignCommand(oFloor106, ActionPlayAnimation(ANIMATION_PLACEABLE_ACTIVATE));
					UT_SetTalkedToBooleanFlag(oFloor106, FALSE);
				}
				else
					{
						AssignCommand(oFloor106, ActionPlayAnimation(ANIMATION_PLACEABLE_DEACTIVATE));
						UT_SetTalkedToBooleanFlag(oFloor106, TRUE);
					}
