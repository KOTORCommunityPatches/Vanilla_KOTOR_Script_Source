		...
		while ((GetIsObjectValid(oNearest) && (GetDistanceBetween(oNearest, oWP) <= floatParam2))) {
			{
				...
				if ((GetIsEnemy(oNearest, oPC) || (intParam3 == 1))) {
					...
					DelayCommand(float4, ApplyEffectToObject(0, EffectDeath(0, 1, 0), oNearest, 0.0));
				}
				}
				(int2++);
...
void main() {
	float float1 = 1.5;
	int int1;
	DelayCommand(float1, sub1("comp_gas_0", 10.0, 1));
	DelayCommand(float1, sub1("comp_gas_1", 10.0, 1));
	DelayCommand(float1, sub1("comp_gas_2", 10.0, 1));
	DelayCommand(float1, sub1("comp_gas_3", 10.0, 1));
	DelayCommand(float1, sub1("comp_gas_4", 10.0, 1));
