			...
			if ((nRandom == 3)) {
				...
				if ((((nRandom == 4) || (nRandom == 5)) && (GetRacialType(OBJECT_SELF) != 5))) {
					if ((GetGender(OBJECT_SELF) == 0)) {
						...
						ActionDoCommand(sub8(intGLOB_52, 1));
					}
					else {
...
void main() {
	...
	sub5();
	AurPostString("k_drd_spawn01: Go go droid jump to waypoint!", 9, 15, 15.0);
	location location1 = GetLocation(OBJECT_SELF);
	object oWP_DROID_EXTRAS = GetObjectByTag("WP_DROID_EXTRAS", 0);
	...
	while (GetIsObjectValid(oShapeObject)) {
		if ((GetTag(oShapeObject) == "g_assassindrd01")) {
			SetCommandable(1, oShapeObject);
			AssignCommand(oShapeObject, ClearAllActions());
			AssignCommand(oShapeObject, ActionJumpToObject(oWP_DROID_EXTRAS, 1));
		}
