...
void main() {
	...
	object oWp_revan = GetObjectByTag("wp_revan", 0);
	object oRevan_fake = GetObjectByTag("revan_fake", 0);
	object oG_darkpc;
	switch (nParam1) {
		case 0:
			ActionMoveToObject(oWp_revan, 0, 1.0);
			SetMinOneHP(GetFirstPC(), 1);
			oG_darkpc = GetObjectByTag("g_darkpc", 0);
			if (GetIsObjectValid(oG_darkpc)) {
				DelayCommand(1.1, DestroyObject(oG_darkpc, 0.0, 0, 0.0, 0));
			}
			break;
		case 1:
			ChangeToStandardFaction(oRevan_fake, 1);
			DelayCommand(0.2, AssignCommand(oRevan_fake, ActionAttack(GetFirstPC(), 0)));
			DelayCommand(0.3, sub1(OBJECT_INVALID));
			break;
		case 2:
			DestroyObject(oRevan_fake, 0.0, 0, 0.0, 0);
			DestroyObject(GetObjectByTag("SithFlag", 0), 0.0, 0, 0.0, 0);
			DestroyObject(GetObjectByTag("SithFlag", 1), 0.0, 0, 0.0, 0);
			DestroyObject(GetObjectByTag("rev_fog", 0), 0.0, 0, 0.0, 0);
