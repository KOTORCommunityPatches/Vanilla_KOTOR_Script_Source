...
void main() {
	...
	switch (nParam1) {
		case 1:
			{
				object oG_mand_first;
				string string1;
				int int3 = 0;
				int3 = 0;
				while ((int3 < 6)) {
				oG_mand_first = GetObjectByTag("g_mand_first", int3);
				string1 = "wp_mand";
				if ((int3 != 0)) {
					string1 = (string1 + IntToString(int3));
				}
					SetCommandable(1, oG_mand_first);
					AssignCommand(oG_mand_first, ClearAllActions());
					AssignCommand(oG_mand_first, ActionMoveToObject(GetObjectByTag(string1, 0), 1, 1.0));
					AssignCommand(oG_mand_first, ActionDoCommand(DelayCommand((1.0 / (int3 + 1)), sub1(OBJECT_INVALID))));
					(int3++);
				}
			}
			break;
		case 2:
			{
				...
				int int4 = 0;
				while ((int4 < 6)) {
				object4 = GetObjectByTag("g_mand_first", int4);
				string3 = "wp_mand";
				if ((int4 != 0)) {
					string3 = (string3 + IntToString(int4));
				}
				object5 = GetObjectByTag(string3, 0);
				if ((GetDistanceBetween(object4, object5) > 2.0)) {
					AurPostString(("a_manatk_cs: Teleport to " + string3), 5, (5 + int4), 5.0);
					AssignCommand(object4, ClearAllActions());
					AssignCommand(object4, ActionJumpToLocation(GetLocation(object5)));
				}
		...
		case 3:
			{
				...
				int int5 = 0;
				while ((int5 < 6)) {
				object8 = GetObjectByTag("g_mand_first", int5);
					ChangeToStandardFaction(object8, 4);
					AssignCommand(object8, ActionDoCommand(sub1(OBJECT_INVALID)));
					(int5++);
				}
			}
			break;
