...
void main() {
	...
	if ((!sub1(int1))) {
		...
		float float1 = 120.0;
		int int5 = GetSkillRank(7, OBJECT_SELF);
		if ((int5 > 0)) {
			float1 = (float1 + (2.0 * int5));
		}
		ApplyEffectToObject(1, efAttackInc, GetSpellTargetObject(), float1);
		if ((((int1 == 75) || (int1 == 76)) || (int1 == 77))) {
			ApplyEffectToObject(1, effect1, GetSpellTargetObject(), float1);
		}
	}
}
