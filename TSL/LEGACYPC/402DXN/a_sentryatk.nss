...
void main() {
	object oMandalorian_sentry;
	object oMandalorian_fr;
	oMandalorian_sentry = GetObjectByTag("mandalorian_sentry", 0);
	int int1 = 0;
	while ((int1 <= 5)) {
		oMandalorian_fr = GetObjectByTag("mandalorian_fr", int1);
		if (GetIsObjectValid(oMandalorian_fr)) {
			DelayCommand(0.2, ChangeToStandardFaction(oMandalorian_fr, 1));
			DelayCommand(0.2, AssignCommand(oMandalorian_fr, ActionAttack(GetFirstPC(), 0)));
		}
		(int1++);
	}
	SetGlobalNumber("402DXN_Sentry_Killer", 1);
