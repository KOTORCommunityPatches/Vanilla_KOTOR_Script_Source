...
void main() {
	...
	int int1 = 0;
	while ((int1 < 8)) {
		oRodianthug = GetObjectByTag("bh_thug1", int1);
		if (GetIsObjectValid(oRodianthug)) {
			DelayCommand(0.1, AssignCommand(oRodianthug, ActionMoveToObject(GetObjectByTag("wp_bh1_run1", 0), 1, 1.0)));
			DelayCommand(0.2, ChangeToStandardFaction(oRodianthug, 1));
			DelayCommand(0.3, AssignCommand(oRodianthug, sub1(OBJECT_INVALID)));
		}
		(int1++);
	}
	int1 = 0;
	while ((int1 < 8)) {
		oRodianthug = GetObjectByTag("rodianthug", int1);
		if (GetIsObjectValid(oRodianthug)) {
			DelayCommand(0.2, ChangeToStandardFaction(oRodianthug, 1));
		}
		DelayCommand(0.3, AssignCommand(oRodianthug, sub1(OBJECT_INVALID)));
