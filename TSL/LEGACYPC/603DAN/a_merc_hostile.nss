...
void main() {
	...
	AssignCommand(oNpc_azkul, sub1(intGLOB_52, 0));
	AssignCommand(oNpc_azkul, SetFacingPoint(GetPosition(oPC)));
	object oG_merc_east;
	int int1 = 0;
	while ((int1 <= 5)) {
		oG_merc_east = GetObjectByTag("g_merc_east", int1);
		if (GetIsObjectValid(oG_merc_east)) {
			AssignCommand(oG_merc_east, sub1(intGLOB_52, 0));
			AssignCommand(oG_merc_east, SetFacingPoint(GetPosition(oPC)));
		}
		(int1++);
	}
}
