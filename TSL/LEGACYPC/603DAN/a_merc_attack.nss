		...
		case 1:
			{
				...
				AssignCommand(oNpc_azkul, ActionMoveToObject(GetObjectByTag("sw_azkul_exit", 0), 0, 1.0));
			}
			break;
		case 2:
			{
				...
				int int3 = 0;
				while ((int3 <= 5)) {
				oG_merc_east = GetObjectByTag("g_merc_east", int3);
				if (GetIsObjectValid(oG_merc_east)) {
					DelayCommand(0.1, ChangeToStandardFaction(oG_merc_east, 1));
					DelayCommand(0.1, AssignCommand(oG_merc_east, ActionAttack(GetFirstPC(), 0)));
				}
					(int3++);
				}
				DelayCommand(0.2, sub1(OBJECT_INVALID));
				object object7 = GetObjectByTag("npc_azkul", 0);
				DelayCommand(0.3, DestroyObject(object7, 0.0, 0, 0.0, 0));
			}
			break;
	}
}
