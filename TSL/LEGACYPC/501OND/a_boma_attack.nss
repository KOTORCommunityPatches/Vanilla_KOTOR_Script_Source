...
void main() {
	...
	switch (nParam1) {
		case 0:
			SetLocalBoolean(oBoma_frenzy, 59, 0);
			DelayCommand(1.8, AssignCommand(oBoma_frenzy, PlayAnimation(10282, 1.0, 0.0)));
			DelayCommand(2.0, PlaySound("bf_large_hard"));
			DelayCommand(2.0, AssignCommand(GetObjectByTag("CageLong", 0), ActionPlayAnimation(202, 1.0, 1.0)));
			break;
		case 1:
			AssignCommand(oBoma_frenzy, JumpToObject(GetObjectByTag("wp_bstrdr_return", 0), 1));
			DelayCommand(0.5, AssignCommand(oBoma_frenzy, ActionMoveToObject(GetFirstPC(), 1, 1.0)));
			break;
		case 2:
			ChangeToStandardFaction(oBoma_frenzy, 1);
			DelayCommand(0.3, AssignCommand(oBoma_frenzy, sub1(OBJECT_INVALID)));
			break;
	}
}
