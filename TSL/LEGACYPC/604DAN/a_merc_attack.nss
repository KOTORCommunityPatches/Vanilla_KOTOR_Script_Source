...
void main() {
	...
	int int1 = 0;
	while ((int1 <= 4)) {
		oG_merc = GetObjectByTag("g_merc", int1);
		if (GetIsObjectValid(oG_merc)) {
			DelayCommand(0.2, ChangeToStandardFaction(oG_merc, 1));
			DelayCommand(0.2, AssignCommand(oG_merc, ActionAttack(GetFirstPC(), 0)));
		}
		(int1++);
	}
	DelayCommand(0.2, ChangeToStandardFaction(oG_merc_capt, 1));
