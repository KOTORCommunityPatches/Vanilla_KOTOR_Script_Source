...
void main() {
	...
	switch (nParam1) {
		case 0:
			...
			DelayCommand(0.5, AssignCommand(oAtris, ActionJumpToObject(GetWaypointByTag("wp_atris_atrend1"), 1)));
			break;
		case 1:
			SetCommandable(1, oAtris);
			SetLocalBoolean(oAtris, intGLOB_114, 0);
			SetNPCSelectability(4, 0);
			ChangeToStandardFaction(oAtris, 1);
			AssignCommand(oAtris, DelayCommand(1.0, AssignCommand(oAtris, sub1(GetFirstPC()))));
			AssignCommand(oHandmaiden, SetIsDestroyable(0, 1, 0));
			effect efDeath = EffectDeath(0, 0, 1);
			ApplyEffectToObject(2, efDeath, oHandmaiden, 0.0);
			break;
		case 2:
			AssignCommand(GetFirstPC(), ActionJumpToObject(GetWaypointByTag("wp_hand_sisend"), 1));
			SetLightsaberPowered(GetFirstPC(), 1, 1, 0);
			break;
		case 3:
			break;
		case 4:
			{
				...
				ApplyEffectToObject(1, efBeam, oHandmaiden, 1.4);
				ApplyEffectToObject(1, efVisual, oHandmaiden, 1.4);
				AssignCommand(oAtris, PlayAnimation(121, 1.0, 0.0));
			}
			break;
		case 5:
			SetGlobalFadeIn(0.3, 2.0, 0.0, 0.0, 0.0);
			DelayCommand(0.5, AssignCommand(oAtris, ActionJumpToObject(GetWaypointByTag("sp_traya_run"), 1)));
			DelayCommand(0.5, AssignCommand(oAtris, ActionMoveToObject(GetWaypointByTag("wp_traya_run"), 1, 1.0)));
			break;
		case 6:
			SetGlobalFadeOut(0.1, 2.0, 0.0, 0.0, 0.0);
			DelayCommand(3.0, sub102());
			AssignCommand(GetFirstPC(), DelayCommand(5.0, SetGlobalFadeIn(0.1, 2.0, 0.0, 0.0, 0.0)));
			DelayCommand(3.8, DestroyObject(oAtris, 0.0, 0, 0.0, 0));
			break;
		case 7:
			DestroyObject(GetObjectByTag("Atris", 0), 0.0, 0, 0.0, 0);
			DestroyObject(GetObjectByTag("Handmaiden", 0), 0.0, 0, 0.0, 0);
			{
				object oN_darthtraya002 = CreateObject(1, "n_darthtraya002", GetLocation(GetWaypointByTag("sp_traya")), 0);
				AssignCommand(GetFirstPC(), ActionJumpToObject(GetWaypointByTag("sp_pc_atrend2"), 1));
				object object22 = SpawnAvailableNPC(4, GetLocation(GetWaypointByTag("sp_hand_atrend1")));
				SetMinOneHP(object22, 0);
				SetMinOneHP(GetFirstPC(), 0);
				SetLockOrientationInDialog(object22, 1);
				effect efHeal = EffectHeal(GetMaxHitPoints(GetFirstPC()));
				ApplyEffectToObject(0, efHeal, GetFirstPC(), 0.0);
				AssignCommand(oN_darthtraya002, ActionStartConversation(GetFirstPC(), "atrend2", 0, 0, 0, "", "", "", "", "", "", 0, 0xFFFFFFFF, 0xFFFFFFFF, 0));
			}
			break;
	}
}
