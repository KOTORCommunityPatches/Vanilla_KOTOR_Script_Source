...
void main() {
	...
	switch (nParam1) {
		case 0:
			sub1("npc_col_tobin", 0);
			int3 = 1;
			while ((int3 <= 5)) {
				sub1("vaklu_troop", int3);
				(int3++);
			}
			break;
		case 1:
			oAreaObject = GetObjectByTag("col_tobin", 0);
			AssignCommand(oAreaObject, ActionMoveToObject(GetObjectByTag("wp_end_tobin", 0), 0, 1.0));
			oAreaObject = GetObjectByTag("vaklu_troop", 0);
			DelayCommand(1.0, AssignCommand(oAreaObject, ActionMoveToObject(GetObjectByTag("wp_vaklu_troop_end1", 0), 0, 1.0)));
			oAreaObject = GetObjectByTag("vaklu_troop", 1);
			DelayCommand(1.0, AssignCommand(oAreaObject, ActionMoveToObject(GetObjectByTag("wp_vaklu_troop_end2", 0), 0, 1.0)));
			oAreaObject = GetObjectByTag("vaklu_troop", 2);
			DelayCommand(2.0, AssignCommand(oAreaObject, ActionMoveToObject(GetObjectByTag("wp_vaklu_troop_end3", 0), 0, 1.0)));
			oAreaObject = GetObjectByTag("vaklu_troop", 3);
			DelayCommand(3.0, AssignCommand(oAreaObject, ActionMoveToObject(GetObjectByTag("wp_vaklu_troop_end4", 0), 0, 1.0)));
			oAreaObject = GetObjectByTag("vaklu_troop", 4);
			DelayCommand(3.0, AssignCommand(oAreaObject, ActionMoveToObject(GetObjectByTag("wp_vaklu_troop_end5", 0), 0, 1.0)));
			break;
		case 2:
			oAreaObject = GetObjectByTag("vaklu_troop", 0);
			int3 = 0;
			while ((int3 <= 4)) {
				object2 = GetObjectByTag("vaklu_troop", int3);
				efStun = EffectStunned();
				ApplyEffectToObject(0, efStun, object2, 3.0);
				(int3++);
			}
			ActionForceMoveToObject(GetObjectByTag("sw_exit", 0), 1, 1.0, 30.0);
			ActionDoCommand(SetCommandable(1, OBJECT_SELF));
			break;
		case 3:
			oAreaObject = GetObjectByTag("col_tobin", 0);
			AssignCommand(oAreaObject, ClearAllEffects());
			AssignCommand(oAreaObject, ClearAllActions());
			AssignCommand(oAreaObject, ActionForceMoveToObject(GetObjectByTag("From_502OND", 0), 1, 1.0, 30.0));
			ActionDoCommand(SetCommandable(1, OBJECT_SELF));
			break;
		case 4:
			int3 = 0;
			while ((int3 <= 4)) {
				object2 = GetObjectByTag("vaklu_troop", int3);
				AssignCommand(object2, ClearAllEffects());
				ChangeToStandardFaction(object2, 1);
				AssignCommand(object2, sub2(OBJECT_INVALID));
				(int3++);
			}
			oAreaObject = GetObjectByTag("col_tobin", 0);
			AssignCommand(oAreaObject, ClearAllActions());
			ActionDoCommand(DestroyObject(oAreaObject, 0.0, 0, 0.0, 0));
			AssignCommand(oAreaObject, SetCommandable(0, OBJECT_SELF));
		...
		case 6:
			oAreaObject = GetFirstObjectInArea(OBJECT_INVALID, 1);
			while (GetIsObjectValid(oAreaObject)) {
				if ((GetTag(oAreaObject) == "vaklu_troop")) {
			...
			break;
		case 7:
			AssignCommand(GetFirstPC(), ActionMoveToObject(GetObjectByTag("sw_exit", 0), 0, 1.0));
			break;
		case 8:
			SetGlobalNumber("500OND_DarkSide_Iziz", 1);
			StartNewModule("512OND", "From_503OND", "", "", "", "", "", "");
			break;
		case 10:
			SetGlobalNumber("500OND_DarkSide_Iziz", 1);
			break;
