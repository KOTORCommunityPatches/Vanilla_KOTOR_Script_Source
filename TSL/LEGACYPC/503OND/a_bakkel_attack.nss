...
void main() {
	...
	int int1 = 0;
	while ((int1 <= 3)) {
		oNpc_bthug = GetObjectByTag("npc_bthug", int1);
		if (GetIsObjectValid(oNpc_bthug)) {
			DelayCommand(0.2, ChangeToStandardFaction(oNpc_bthug, 1));
			DelayCommand(0.2, AssignCommand(oNpc_bthug, ActionAttack(GetFirstPC(), 0)));
		}
		(int1++);
	}
	oBakkel = GetObjectByTag("bakkel", 0);
