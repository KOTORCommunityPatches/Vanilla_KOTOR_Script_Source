...
void main() {
	...
	int1 = 0;
	while ((int1 <= 4)) {
		oVaklu_troop = GetObjectByTag("vaklu_troop", int1);
		AssignCommand(oVaklu_troop, ClearAllEffects());
		ChangeToStandardFaction(oVaklu_troop, 1);
		AssignCommand(oVaklu_troop, sub1(OBJECT_INVALID));
		(int1++);
	}
	object oCol_tobin = GetObjectByTag("col_tobin", 0);
	AssignCommand(oCol_tobin, ClearAllActions());
	ActionDoCommand(DestroyObject(oCol_tobin, 0.0, 0, 0.0, 0));
	AssignCommand(oCol_tobin, SetCommandable(0, OBJECT_SELF));
