...
// Prototypes
void sub104(string stringParam1);
int sub103(object objectParam1);
	...
	if ((GetIsObjectValid(oTarget) && (!GetIsDead(oTarget)))) {
		...
		if ((fDistance < 3.5)) {
			...
			if (GetIsInCombat(OBJECT_SELF, 0)) {
				DelayCommand(3.0, sub4(objectParam1));
			}
		}
		else {
			int nAction = GetCurrentAction(OBJECT_SELF);
...
void main() {
	...
	if ((int1 == 1001)) {
		if (GetLocalBoolean(OBJECT_SELF, 50)) {
			location location1 = sub1(GetFirstPC(), 2.5);
					...
					if ((int1 == 1005)) {
						...
						if ((int1 == 1006)) {
							...
							if ((int1 == 1007)) {
								...
								int int6 = 15;
								int int7 = 0;
								location location3 = GetLocation(OBJECT_SELF);
								float fFacing = GetFacing(OBJECT_SELF);
