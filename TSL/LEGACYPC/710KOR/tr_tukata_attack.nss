...
void main() {
	...
	location location1 = GetLocation(GetNearestObjectByTag("wp_tukata_attack", OBJECT_SELF, 1));
	object oShapeObject = GetFirstObjectInShape(4, 30.0, location1, 0, 1, [0.0,0.0,0.0]);
	while (GetIsObjectValid(oShapeObject)) {
		if ((GetTag(oShapeObject) == "g_tukata02")) {
			ChangeToStandardFaction(oShapeObject, 3);
			AssignCommand(oShapeObject, sub1(OBJECT_INVALID));
		}
		oShapeObject = GetNextObjectInShape(4, 30.0, location1, 0, 1, [0.0,0.0,0.0]);
	}
}
