...
void main() {
	...
	switch (nParam1) {
		case 0:
			oTurret_iziz = GetObjectByTag("cp_replace", 0);
			location1 = GetGlobalLocation("k2_temp_loc");
			AssignCommand(oTurret_iziz, ActionMoveToLocation(location1, 0));
			ActionDoCommand(AssignCommand(oTurret_iziz, SetFacing(270.0)));
			break;
		case 1:
			SetLocalBoolean(OBJECT_SELF, 10, 1);
			DelayCommand(2.5, ActionStartConversation(GetFirstPC(), "", 0, 0, 0, "", "", "", "", "", "", 0, 0xFFFFFFFF, 0xFFFFFFFF, 0));
			break;
		case 2:
			int3 = 0;
			while ((int3 <= 3)) {
				oTurret_iziz = GetObjectByTag("vaklu_common", int3);
				if ((int3 < 2)) {
					AssignCommand(oTurret_iziz, ActionEquipMostDamagingRanged(OBJECT_INVALID));
				}
				else {
					AssignCommand(oTurret_iziz, ActionEquipMostDamagingMelee(OBJECT_INVALID, 0));
				}
				AssignCommand(oTurret_iziz, ActionDoCommand(ChangeToStandardFaction(oTurret_iziz, 1)));
				AssignCommand(oTurret_iziz, ActionAttack(GetPartyMemberByIndex((int3 % 3)), 0));
				AssignCommand(oTurret_iziz, ActionDoCommand(sub1(OBJECT_INVALID)));
				(int3++);
			}
			break;
		case 3:
			int3 = 0;
			while ((int3 <= 8)) {
				oTurret_iziz = GetObjectByTag("turret_iziz", int3);
				ChangeToStandardFaction(oTurret_iziz, 1);
				AssignCommand(oTurret_iziz, sub1(OBJECT_INVALID));
				(int3++);
			}
			break;
	}
