...
// Prototypes
void sub102(string stringParam1);
...
void main() {
	...
	if ((int1 == 1001)) {
		AurPostString("Heart beating", 5, 9, 5.0);
		if (GetLocalBoolean(GetObjectByTag("utildroid_mk3", 0), 30)) {
			AurPostString("Moving droids to PC's location", 5, 10, 5.0);
		}
		sub1(OBJECT_INVALID);
		ActionForceMoveToLocation(GetLocation(GetFirstPC()), 0, 30.0);
	}
	else {
		if ((int1 == 1002)) {
		}
