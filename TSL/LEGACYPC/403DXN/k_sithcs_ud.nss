...
void main() {
	...
	if ((int1 == 1001)) {
		if (GetMinOneHP(OBJECT_SELF)) {
			if ((GetDistanceToObject2D(GetFirstPC()) < 20.0)) {
		...
		if ((int1 == 1002)) {
			...
			if ((GetLastPerceptionSeen() && GetIsPartyLeader(oPerceived))) {
				SetMinOneHP(OBJECT_SELF, 0);
			}
		}
		else {
