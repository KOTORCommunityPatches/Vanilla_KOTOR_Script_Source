...
void main() {
	...
	if ((int2 == 1003)) {
		...
		if (((nCurHP < 5) && (!GetLocalBoolean(OBJECT_SELF, 42)))) {
			...
			while (GetIsObjectValid(oCrypt_door)) {
				...
				(int1++);
				oCrypt_door = GetObjectByTag("crypt_door", int1);
			}
		...
		if ((int2 == 1006)) {
			if (((GetCurrentHitPoints(OBJECT_SELF) < 5) && (!GetLocalBoolean(OBJECT_SELF, 40)))) {
				...
				CancelCombat(GetFirstPC());
				ChangeToStandardFaction(OBJECT_SELF, 5);
				DelayCommand(1.0, AssignCommand(OBJECT_SELF, ActionPlayAnimation(26, 1.0, (-1.0))));
				NoClicksFor(4.0);
				IncrementGlobalNumber("711KOR_Vision_Track", 1);
				SetMinOneHP(GetFirstPC(), 0);
				int1 = 0;
				object object12 = GetObjectByTag("crypt_door", 0);
				while (GetIsObjectValid(object12)) {
					SetLocked(object12, 0);
					SetPlotFlag(object12, 0);
					(int1++);
					object12 = GetObjectByTag("crypt_door", int1);
				}
				object object15 = GetObjectByTag("rev_end", 0);
				if ((!GetIsObjectValid(object15))) {
					return;
				}
				AssignCommand(object15, ClearAllActions());
				AssignCommand(object15, ActionWait(2.0));
				AssignCommand(object15, ActionStartConversation(GetFirstPC(), "kreia_tm", 0, 0, 1, "", "", "", "", "", "", 0, 0xFFFFFFFF, 0xFFFFFFFF, 0));
				DelayCommand(0.5, DestroyObject(OBJECT_SELF, 0.0, 0, 0.0, 0));
			}
		}
		else {
			if ((int2 == intGLOB_149)) {
