...
void main() {
	...
	int int1 = (FloatToInt((GetMaxHitPoints(oPC) / 2.0)) - GetCurrentHitPoints(oPC));
	effect efVisual = EffectVisualEffect(1012, 0);
	if ((GetIsObjectValid(oRevan_fake) && (int1 > 0))) {
		effect efHeal = EffectHeal(int1);
		AssignCommand(oRevan_fake, ActionCastFakeSpellAtObject(38, oPC, 0));
		DelayCommand(1.0, ApplyEffectToObject(1, efVisual, oPC, 3.0));
		DelayCommand(1.0, ApplyEffectToObject(0, efHeal, oPC, 0.0));
		DelayCommand(2.0, sub1(oPC));
	}
}