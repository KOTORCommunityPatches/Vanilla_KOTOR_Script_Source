...
void main() {
	...
	switch (nParam1) {
		...
		case 8:
			...
			if (((GetGlobalNumber("000_Hand_Finale") == 1) && (GetGender(GetFirstPC()) == 0))) {
				SetGlobalNumber("000_Handmaiden_Dead", 5);
				DelayCommand(5.0, sub1());
				DelayCommand(5.0, AssignCommand(oKreiaEvil, ActionStartConversation(GetFirstPC(), "krehand1", 0, 0, 0, "", "", "", "", "", "", 0, 0xFFFFFFFF, 0xFFFFFFFF, 0)));
			}
			else {
			...
			break;
		case 9:
			AssignCommand(oAtris, ActionPlayAnimation(19, 1.0, 1.0));
	}
}