...
void main() {
	...
	if ((float5 <= 0.3)) {
		...
		CancelCombat(oPC);
		ApplyEffectToObject(2, EffectStunned(), OBJECT_SELF, 0.0);
		IncrementGlobalNumber("262TEL_Sis_Stun", 1);
		int nGlobal = GetGlobalNumber("262TEL_Sis_Stun");
		ExecuteScript("k_ai_master", OBJECT_SELF, intGLOB_6);
		if ((GetGlobalNumber("262TEL_Sis_Stun") == 5)) {
			object object3 = GetFirstPC();
			int int6 = (GetMaxHitPoints(object3) - GetCurrentHitPoints(object3));
			ApplyEffectToObject(0, EffectHeal(int6), object3, 0.0);
