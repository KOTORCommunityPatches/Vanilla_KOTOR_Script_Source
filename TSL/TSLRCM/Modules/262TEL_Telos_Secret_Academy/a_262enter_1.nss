	...
	switch (nParam1) {
		...
		case 2:
			...
			vector vPosition = GetPosition(GetObjectByTag("Handmaiden", 0));
			{
				...
				AssignCommand(object36, ActionDoCommand(SetFacingPoint(vPosition)));
				AssignCommand(object32, ClearAllActions());
				AssignCommand(object32, ActionWait(0.5));
				AssignCommand(object32, ActionDoCommand(SetFacingPoint(vPosition)));
				AssignCommand(object32, ActionDoCommand(SetLockOrientationInDialog(object32, 0)));
				AssignCommand(object34, ClearAllActions());
				AssignCommand(object34, ActionWait(0.8));
				AssignCommand(object34, ActionDoCommand(SetFacingPoint(vPosition)));
				AssignCommand(object34, ActionDoCommand(SetLockOrientationInDialog(object34, 0)));
			}
			break;
	}
}