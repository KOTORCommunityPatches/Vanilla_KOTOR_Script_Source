			...
			if ((nRandom == 3)) {
				...
				if ((((nRandom == 4) || (nRandom == 5)) && (GetRacialType(OBJECT_SELF) != 5))) {
					if ((GetGender(OBJECT_SELF) == 0)) {
						...
						ActionDoCommand(sub2(intGLOB_79, 1));
					}
					else {
...
int sub84(object objectParam1) {
	...
	if ((sub85() != (-1))) {
		...
		if (int8) {
			...
			if (sub15(intGLOB_155, GetFirstPC())) {
				return 1;
			}
		}
	}
	if (GetIsObjectValid(object1)) {
		if (sub15(intGLOB_152, object1)) {
			return 1;
		}
	}
	object object9 = sub14();
	if (GetIsObjectValid(object9)) {
		if (sub15(intGLOB_153, object9)) {
			return 1;
		}
	}
	if (sub15(intGLOB_154, OBJECT_SELF)) {
		return 1;
	}
	object3 = sub29();
...
int sub33(object objectParam1) {
	...
	oNPC = sub34();
	if (GetIsObjectValid(oNPC)) {
		if (sub15(intGLOB_152, oNPC)) {
			return 1;
		}
	}
	oNPC = sub14();
	if (GetIsObjectValid(oNPC)) {
		if (sub15(intGLOB_153, oNPC)) {
			return 1;
		}
	}
	talent1 = sub35(0);
...
void sub1(object objectParam1, int intParam2) {
	if ((intParam2 == 0)) {
		AurPostString("droid hostility", 5, 5, 5.0);
		...
		if ((intParam2 == 2)) {
			ClearAllActions();
			sub2(intGLOB_120, 1);
			AurPostString("Come follow, follow, follow, follow, follow, follow me!", 5, 3, 10.0);
			DelayCommand(5.0, AurPostString("Whither shall I follow, follow, follow? Whither shall I follow, follow thee?", 5, 5, 10.0));
			DelayCommand(10.0, AurPostString("To the redwood, to the redwood, to the redwood, redwood tree!", 5, 7, 10.0));
		}
		else {
			if ((intParam2 == 1)) {
				AurPostString("droids go insane, got no brain", 5, 5, 5.0);
				ChangeToStandardFaction(objectParam1, 6);
			}
		}
	}
}
...
void main() {
	...
	float float1 = IntToFloat(nParam1);
	while (GetIsObjectValid(oAreaObject)) {
		if (((FindSubString(GetTag(oAreaObject), "utildroid") >= 0) && (oAreaObject != OBJECT_SELF))) {
			DelayCommand(float1, AssignCommand(oAreaObject, ActionDoCommand(sub1(oAreaObject, nParam2))));
			DelayCommand(float1, AssignCommand(oAreaObject, ActionDoCommand(sub3(OBJECT_INVALID))));
		}
