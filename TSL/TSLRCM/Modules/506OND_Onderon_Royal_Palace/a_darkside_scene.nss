...
void main() {
	...
	switch (nParam1) {
		...
		case 3:
			...
			DelayCommand(0.1, ChangeToStandardFaction(oDs_sith_elite_sp, 3));
			CreateObject(1, "c_maalraas1", GetLocation(GetObjectByTag("sp_va_speaker_1", 0)), 0);
			CreateObject(1, "c_boma2", GetLocation(GetObjectByTag("sp_va_sold_gren001", 0)), 0);
			CreateObject(1, "ds_si_jedi_appr", GetLocation(GetObjectByTag("sp_va_sold_gun0012", 0)), 0);
			break;
	}
}