...
void main() {
	...
	IncrementGlobalNumber("601DAN_Dead_Militia", 1);
	if ((GetGlobalNumber("601DAN_Side") == 2)) {
		if ((GetGlobalNumber("601DAN_Dead_Militia") == 11)) {
			AurPostString("BEGIN RETREAT SEQUENCE", 5, 20, 10.0);
	...
	if ((GetGlobalNumber("601DAN_Side") == 1)) {
		if ((GetGlobalNumber("601DAN_Dead_Militia") == 10)) {
			AurPostString("BEGIN RETREAT SEQUENCE", 5, 20, 10.0);
			SetGlobalNumber("601DAN_Battle_Mili", 2);
			SetGlobalNumber("601DAN_Battle_Start", 2);
			ExecuteScript("a_mil_retreat", GetFirstPC(), 0xFFFFFFFF);
