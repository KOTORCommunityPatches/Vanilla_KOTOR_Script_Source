...
void main() {
	...
	object oWP_front_door = GetObjectByTag("WP_front_door", 0);
	...
	switch (nParam1) {
		...
		case 3:
			...
			AssignCommand(oPC, ActionForceMoveToLocation(GetLocation(oWP_front_door), 1, 30.0));
			if (GetIsObjectValid(oNPC)) {
				AssignCommand(oNPC, ClearAllActions());
				AssignCommand(oNPC, ActionForceMoveToLocation(GetLocation(oWP_front_door), 1, 30.0));
			}
			if (GetIsObjectValid(object7)) {
				AssignCommand(object7, ClearAllActions());
				AssignCommand(object7, ActionForceMoveToLocation(GetLocation(oWP_front_door), 1, 30.0));
			}
			break;
		case 4:
			AssignCommand(oPC, ClearAllActions());
		...
		case 5:
			AssignCommand(oNpc_zherron, ClearAllActions());
			AssignCommand(oNpc_zherron, ActionForceMoveToLocation(GetLocation(oWP_front_door), 1, 30.0));
			sub1(oWP_front_door);
			break;
		case 6:
