			...
			if ((nRandom == 3)) {
				...
				if ((((nRandom == 4) || (nRandom == 5)) && (GetRacialType(OBJECT_SELF) != 5))) {
					if ((GetGender(OBJECT_SELF) == 0)) {
						...
						ActionDoCommand(sub11(intGLOB_52, 1));
					}
					else {
...
void sub99() {
	if ((!sub8(intGLOB_61))) {
		...
		if (GetIsObjectValid(GetWaypointByTag(string11))) {
			...
			ActionForceMoveToObject(object3, sub8(intGLOB_59), fPersonalSpace, float4);
			if (sub8(intGLOB_52)) {
...
int sub96() {
	...
	if ((sub8(intGLOB_112) && (!sub8(intGLOB_113)))) {
		if (sub97()) {
			return 1;
		}
	}
	...
	if (((!IsObjectPartyMember(OBJECT_SELF)) && (sub8(intGLOB_61) == 1))) {
	...
	if ((GetIsObjectValid(oTarget) && (!GetIsDead(oTarget)))) {
		...
		if ((fDistance < 3.5)) {
			...
			if (GetIsInCombat(OBJECT_SELF, 0)) {
				DelayCommand(3.0, sub3(intParam1));
			}
		}
		else {
			int nAction = GetCurrentAction(OBJECT_SELF);
...
int sub34(object objectParam1) {
	...
	if ((nLocalBool == 1)) {
		...
		if ((GetNPCAIStyle(OBJECT_SELF) == 10)) {
			...
			if ((GetNPCAIStyle(OBJECT_SELF) != intGLOB_116)) {
	...
	if (((float12 > 30.0) && (GetNPCAIStyle(OBJECT_SELF) != intGLOB_116))) {
		oNPC = OBJECT_INVALID;
		CancelCombat(OBJECT_SELF);
	}
	if (GetIsObjectValid(oNPC)) {
...
int sub12() {
	int int1 = 0;
	if (((sub8(intGLOB_119) == 0) && (!IsObjectPartyMember(OBJECT_SELF)))) {
		if (((GetHitDice(GetFirstPC()) >= 15) || sub8(intGLOB_116))) {
...
int sub9() {
	...
	sub4(((("GENERIC DEBUG *************** Shield Boolean (" + sub10(intGLOB_117)) + ") = ") + sub10(sub8(intGLOB_117))));
	if (((sub8(intGLOB_117) == 0) && (!IsObjectPartyMember(OBJECT_SELF)))) {
		int int4;
		int int5;
		int int6 = 0;
		talent talSpell;
...
void sub3(int intParam1) {
	...
	sub6();
	{
		...
		if (((((!sub8(intGLOB_112)) && (!sub8(intGLOB_113))) && (!sub8(intGLOB_114))) && (!GetUserActionsPending()))) {
		if (((GetPartyMemberByIndex(0) != OBJECT_SELF) && (!GetPlayerRestrictMode(OBJECT_SELF)))) {
			if (((IsObjectPartyMember(OBJECT_SELF) && (!GetPlayerRestrictMode(OBJECT_SELF))) || (!IsObjectPartyMember(OBJECT_SELF)))) {
				...
				if ((sub8(intGLOB_121) == 1)) {
					if ((sub13() == 1)) {
						return;
					}
				}
				if ((sub8(intGLOB_116) == 1)) {
...
void sub2(object objectParam1) {
	...
	DelayCommand(0.6, sub3(OBJECT_INVALID));
...
void main() {
	...
	SetLocalBoolean(oHandmaiden, 26, 0);
	SetLocalBoolean(oHandmaiden, 27, 0);
	SetLocalBoolean(oHandmaiden, 29, 0);
	SetLocalBoolean(oHandmaiden, 28, 1);
	int int3 = 0;
	SetMinOneHP(oPC, 1);
	switch (nParam1) {
		case 0:
			...
			SetGlobalFadeIn(0.5, 2.0, 0.0, 0.0, 0.0);
			{
				...
				AssignCommand(oCargoDoor, ActionCloseDoor(oCargoDoor));
				SetLocked(oCargoDoor, 1);
				int3 = 0;
				while ((int3 < 12)) {
					ChangeToStandardFaction(GetObjectByTag(sub1(int3), 0), 5);
					(int3++);
				}
				AssignCommand(oPC, ActionJumpToObject(GetObjectByTag("WP_HANDFIGHT_PC", 0), 1));
				...
				object object13 = GetObjectByTag("Handmaiden", 0);
				AssignCommand(object13, ActionJumpToObject(GetObjectByTag("WP_HANDFIGHT_HAND", 0), 1));
				AssignCommand(object13, ClearAllEffects());
				GiveItem(GetItemInSlot(5, object13), oPC);
				GiveItem(GetItemInSlot(4, object13), oPC);
				GiveItem(GetItemInSlot(1, object13), object13);
				DelayCommand(1.9, sub2(oHandmaiden));
			}
			break;
	}
}