void main() {
	...
	if ((nParam1 == 1)) {
		...
		AssignCommand(oChamp, ActionForceMoveToLocation(GetLocation(oWp_kodin_1), 0, 30.0));
	}
	else {
		if ((nParam1 == 2)) {
			object oDoor_Warehouse_Inner = GetObjectByTag("Door_Warehouse_Inner", 0);
			AssignCommand(oDoor_Warehouse_Inner, ActionUnlockObject(oDoor_Warehouse_Inner));
			...
			AssignCommand(object13, ActionCloseDoor(object13));
			...
			if ((nParam1 == 3)) {
				...
				DelayCommand(1.1, DestroyObject(GetObjectByTag("Champ", 0), 0.0, 1, 0.0, 0));
			}
		}
	}
}