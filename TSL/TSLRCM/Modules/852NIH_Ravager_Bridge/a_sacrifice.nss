void main() {
	...
	object oZbyls_nih = GetObjectByTag("zbyls_nih", 0);
	...
	switch (nParam1) {
		case 0:
			...
			DelayCommand(1.0, SetLightsaberPowered(oZbyls_nih, 1, 0, 1));
			...
			AssignCommand(oZbyls_nih, ActionJumpToObject(GetObjectByTag("wp_nihilus_dead", 0), 1));
			SetMinOneHP(oDarthNihilus, 0);
			SetMinOneHP(oZbyls_nih, 0);
			AssignCommand(oDarthNihilus, SetFacing(0.0));
			AssignCommand(oZbyls_nih, SetFacing(0.0));
			AssignCommand(oEnd, DelayCommand(1.1, AssignCommand(oDarthNihilus, ClearAllActions())));
			AssignCommand(oEnd, DelayCommand(1.1, AssignCommand(oZbyls_nih, ClearAllActions())));
			AssignCommand(oEnd, DelayCommand(1.1, AssignCommand(oDarthNihilus, ActionPlayAnimation(26, 1.0, (-1.0)))));
			AssignCommand(oEnd, DelayCommand(1.1, AssignCommand(oZbyls_nih, ActionPlayAnimation(26, 1.0, (-1.0)))));
			SetLockOrientationInDialog(oDarthNihilus, 1);
			SetLockOrientationInDialog(oZbyls_nih, 1);
		...
		case 4:
			...
			ActionPauseConversation();
			{
				...
				DestroyObject(oZbyls_nih, 0.0, 0, 0.0, 0);
