...
void main() {
	...
	if (((GetGlobalNumber("203TEL_B-4D4_PC") == 1) && GetIsObjectValid(GetObjectByTag("dev_killer", 0)))) {
		DestroyObject(GetObjectByTag("rod_killer", 0), 0.0, 0, 0.0, 0);
		DestroyObject(GetObjectByTag("dev_killer", 0), 0.0, 0, 0.0, 0);
		SetGlobalNumber("203TEL_BH_CS", 1);
	}
	else {
		if (((GetGlobalNumber("203TEL_BH_CS") == 1) && (!GetIsObjectValid(GetObjectByTag("dev_killer", 0))))) {
