...
void main() {
	...
	int int1 = 0;
	while ((int1 <= 5)) {
		oSelite_run = GetObjectByTag("sgren_run", int1);
		if (GetIsObjectValid(oSelite_run)) {
			DelayCommand(0.2, ChangeToStandardFaction(oSelite_run, 1));
			DelayCommand(0.2, AssignCommand(oSelite_run, ActionAttack(GetFirstPC(), 0)));
		}
		(int1++);
	}
	oSelite_run = GetObjectByTag("selite_run", 0);
	DelayCommand(0.2, ChangeToStandardFaction(oSelite_run, 1));
	DelayCommand(0.2, AssignCommand(oSelite_run, ActionAttack(GetFirstPC(), 0)));
	oSelite_run = GetObjectByTag("selite_run", 1);
	DelayCommand(0.2, ChangeToStandardFaction(oSelite_run, 1));
	DelayCommand(0.2, AssignCommand(oSelite_run, ActionAttack(GetFirstPC(), 0)));
	DelayCommand(0.2, ChangeToStandardFaction(oSelite_run, 1));
