	...
	while ((int1 <= int2)) {
		{
			...
			if (GetIsObjectValid(oSerrocoThugCommon)) {
				...
				AssignCommand(oSerrocoThugCommon, ActionForceMoveToObject(GetObjectByTag("WP_serroco_2", 0), 1, 1.0, 30.0));
			}
			if (GetIsObjectValid(oSerrocoThug1)) {
				AssignCommand(oSerrocoThug1, ClearAllActions());
				AssignCommand(oSerrocoThug1, ActionForceMoveToObject(GetObjectByTag("WP_serroco_2", 0), 1, 1.0, 30.0));
			}
			if (GetIsObjectValid(oSerrocoThugVeteran)) {
				AssignCommand(oSerrocoThugVeteran, ClearAllActions());
				AssignCommand(oSerrocoThugVeteran, ActionForceMoveToObject(GetObjectByTag("WP_serroco_2", 0), 1, 1.0, 30.0));
			}
		}
		(int1++);
	}
	AssignCommand(GetObjectByTag("SerrocoLeader", 0), ClearAllActions());
	AssignCommand(GetObjectByTag("SerrocoLeader", 0), ActionForceMoveToObject(GetObjectByTag("WP_serroco_2", 0), 1, 1.0, 30.0));
