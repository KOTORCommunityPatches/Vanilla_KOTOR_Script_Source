...
string sub2(string stringParam1);
...
void main() {
	object oCage_jedi = GetObjectByTag("cage_jedi", 0);
	if ((!GetLocalBoolean(oCage_jedi, 40))) {
		SetLocalBoolean(oCage_jedi, 40, 1);
		int int2 = 1;
		...
		if ((GetJournalEntry("LightsaberQuest") < 40)) {
			...
			while (GetIsObjectValid(object3)) {
				string2 = (string1 + IntToString(int2));
				object3 = GetItemPossessedBy(GetPartyLeader(), string2);
				(int2++);
			}
			AddJournalQuestEntry("LightsaberQuest", (10 * int2), 0);
		}
		else {
			string2 = sub1(((GetGlobalNumber("G_PC_LEVEL") - 2) + Random(5)), 140);
		}
