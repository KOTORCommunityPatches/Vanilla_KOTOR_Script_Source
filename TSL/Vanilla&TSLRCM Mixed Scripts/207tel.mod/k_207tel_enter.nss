...
void sub2() {
	...
	int int6 = (((GetGlobalNumber("211TEL_SWOOP_MIN") * 6000) + (GetGlobalNumber("211TEL_SWOOP_SEC") * 100)) + GetGlobalNumber("211TEL_SWOOP_MSEC"));
	sub3(55, int6);
	if (((int6 < int1) || (int1 == 0))) {
	...
	if (GetLocalBoolean(oSwoopMC, 61)) {
		return;
	}
...
void main() {
	...
	if ((GetGlobalNumber("207TEL_Dancer_Spawn") == 1)) {
		...
		if ((GetGlobalNumber("203TEL_Ramana") == 4)) {
			...
			DelayCommand(0.3, ExecuteScript("spawnnwdnc", OBJECT_SELF, 0xFFFFFFFF));
		}
		if ((GetGlobalNumber("203TEL_Harra") == 7)) {
			SetGlobalNumber("203TEL_Harra", 8);
			CreateObject(1, "203_harra", GetLocation(GetObjectByTag("wp_viewer_sp", 0)), 0);
		}
		if ((GetGlobalNumber("207TEL_Doton") > 0)) {
			DestroyObject(GetObjectByTag("doton", 0), 0.0, 0, 0.0, 0);
		}
	}
	...
	if ((GetGlobalNumber("200TEL_Falt_Arrest") == 5)) {
		SetGlobalNumber("200TEL_Falt_Arrest", 6);
