...
void main() {
	...
	if (((GetCurrentHitPoints(OBJECT_SELF) <= (GetMaxHitPoints(OBJECT_SELF) / 2)) && (!GetLocalBoolean(OBJECT_SELF, 40)))) {
		...
		ClearAllActions();
		DelayCommand(0.1, ActionPlayAnimation(121, 1.0, 3.0));
		SetGlobalFadeIn(0.0, 2.5, 1.0, 1.0, 1.0);
		AssignCommand(oPC, ClearAllActions());
		effect efVisual = EffectVisualEffect(3001, 0);
		efVisual = EffectLinkEffects(efVisual, EffectForceSight());
		efVisual = EffectLinkEffects(efVisual, EffectCutSceneHorrified());
		efVisual = EffectLinkEffects(efVisual, EffectVisualEffect(1042, 0));
		DelayCommand(0.3, ApplyEffectToObject(1, efVisual, oPC, 6.0));
	}
	else {
