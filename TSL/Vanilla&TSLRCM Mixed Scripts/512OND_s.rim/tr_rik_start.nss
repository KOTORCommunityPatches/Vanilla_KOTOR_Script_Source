...
void main() {
	...
	if ((((GetGlobalNumber("512OND_Riiken_Patrol") == 0) && (GetGlobalNumber("502OND_Riiken_Talked") == 0)) && (GetGlobalNumber("502OND_Dhagon_Free") >= 1))) {
		if ((!GetLocalBoolean(OBJECT_SELF, 45))) {
			SetLocalBoolean(OBJECT_SELF, 45, 1);
			AurPostString("Spawning Riiken", 5, 5, 5.0);
			CreateObject(1, "g_sithcomm003", GetLocation(GetObjectByTag("wp_riiken_amb_1", 0)), 0);
			SetGlobalNumber("512OND_Riiken_Patrol", 1);
		}
		if ((!GetLocalBoolean(OBJECT_SELF, 46))) {
			if ((GetJournalEntry("matter_leader") >= 10)) {
				AurPostString("Starting cutscene showing Riiken", 5, 6, 5.0);
				object oG_riiken = GetObjectByTag("g_riiken", 0);
				AssignCommand(oG_riiken, ClearAllActions());
				AssignCommand(oG_riiken, ActionJumpToObject(GetObjectByTag("wp_riiken_amb_1", 0), 1));
				AssignCommand(oG_riiken, ActionDoCommand(sub1()));
				DelayCommand(1.0, AssignCommand(GetObjectByTag("rik_dummy", 0), ActionStartConversation(GetFirstPC(), "ri_start", 0, 0, 0, "", "", "", "", "", "", 0, 0xFFFFFFFF, 0xFFFFFFFF, 0)));
