void main() {
	...
	if ((nParam1 == 1)) {
		...
		DelayCommand(0.1, AssignCommand(GetObjectByTag("GamGuardLuxB", 0), ActionMoveToObject(GetWaypointByTag("WP_boss_run"), 1, 1.0)));
		...
		if ((nParam1 == 2)) {
			...
			AssignCommand(oDrdBdyGrd, ActionMoveToObject(GetWaypointByTag("WP_DBG1"), 1, 1.0));
			AssignCommand(object16, ActionMoveToObject(GetWaypointByTag("WP_DBG2"), 1, 1.0));
			...
			DelayCommand(2.8, DestroyObject(GetObjectByTag("GamGuardLuxB", 0), 0.0, 1, 0.0, 0));
		}
	}
}
