	...
	if ((GetEnteringObject() == GetFirstPC())) {
		if ((GetGlobalNumber("299TEL_HK51_Active") == 3)) {
			if (GetIsObjectValid(oHk51cs)) {
				...
				AssignCommand(oHk51cs, ActionStartConversation(GetFirstPC(), "hk51main", 0, 0, 1, "", "", "", "", "", "", 0, 0xFFFFFFFF, 0xFFFFFFFF, 0));
			}
			SendMessageToPC(GetFirstPC(), ("HK_Active is at" + IntToString(GetGlobalNumber("299TEL_HK51_Active"))));
