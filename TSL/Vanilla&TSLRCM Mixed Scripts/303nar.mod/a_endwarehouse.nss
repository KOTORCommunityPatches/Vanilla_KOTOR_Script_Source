...
void sub1() {
	...
	SpawnAvailableNPC(8, GetLocation(oWp_t3m4_whend));
}
...
void main() {
	...
	DelayCommand(2.8, YavinHackDoorClose(GetObjectByTag("TO_304NAR", 0)));
	object oDoor_vogga002 = GetObjectByTag("door_vogga002", 0);
	if (GetIsObjectValid(oDoor_vogga002)) {
		DelayCommand(2.8, AssignCommand(oDoor_vogga002, ActionLockObject(oDoor_vogga002)));
		DelayCommand(2.8, AssignCommand(oDoor_vogga002, ActionCloseDoor(oDoor_vogga002)));
	}
