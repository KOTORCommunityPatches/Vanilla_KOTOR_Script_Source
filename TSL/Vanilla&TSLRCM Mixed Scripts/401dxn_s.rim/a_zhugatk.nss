...
void main() {
	...
	int int1 = 0;
	while ((int1 <= 1)) {
		if (GetIsObjectValid(GetObjectByTag("duros_bh", int1))) {
			DelayCommand(0.2, ChangeToStandardFaction(GetObjectByTag("duros_bh", int1), 1));
			DelayCommand(0.2, AssignCommand(GetObjectByTag("duros_bh", int1), ActionAttack(GetFirstPC(), 0)));
		}
		(int1++);
	}
	object oTdroid3 = GetObjectByTag("tdroid", 0);
	if (GetLocalBoolean(oTdroid3, 19)) {
		DelayCommand(0.2, AssignCommand(oTdroid3, ActionAttack(oDezanti, 0)));
	}
