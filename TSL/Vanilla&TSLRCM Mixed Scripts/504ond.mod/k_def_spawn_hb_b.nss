...
void sub18();
string sub17(int intParam1, int intParam2);
...
void sub18() {
	if ((!sub5(intGLOB_61))) {
		...
		if (GetIsObjectValid(GetWaypointByTag(string11))) {
			...
			ActionForceMoveToObject(object3, sub5(intGLOB_59), fPersonalSpace, float4);
			if (sub5(intGLOB_52)) {
...
string sub17(int intParam1, int intParam2) {
	string string1 = sub8(intParam1, intParam2);
	return string1;
}
...
void main() {
	...
	SetLocalNumber(OBJECT_SELF, 28, 1);
	if ((GetTag(OBJECT_SELF) == "roy_sold_pc")) {
		SetLocalBoolean(OBJECT_SELF, 40, 1);
	}
	sub6(OBJECT_SELF, (Random(2) + 1), 0);
	sub18();
}
