				...
				if ((int1 == 1004)) {
					...
					if ((int1 == 1005)) {
						...
						if ((int1 == 1006)) {
							...
							if ((int1 == 1007)) {
								object oMand_merc = GetObjectByTag("mand_merc", 0);
								object oPC = GetFirstPC();
								CancelCombat(oPC);
								SetLocalBoolean(oMand_merc, 40, 1);
								AssignCommand(oMand_merc, ActionStartConversation(GetFirstPC(), "", 0, 0, 0, "", "", "", "", "", "", 0, 0xFFFFFFFF, 0xFFFFFFFF, 0));
							}
							else {
								if ((int1 == 1008)) {
								}
