...
void main() {
	...
	AssignCommand(oPC, ActionJumpToObject(GetWaypointByTag("wp_skypc"), 1));
	DelayCommand(0.1, SetFacing(270.0));
	object object4 = GetNextPC();
	if (GetIsObjectValid(object4)) {
		AssignCommand(object4, ClearAllActions());
		AssignCommand(object4, ActionJumpToObject(GetWaypointByTag("wp_skypc1"), 1));
	}
	object4 = GetNextPC();
	if (GetIsObjectValid(object4)) {
		AssignCommand(object4, ClearAllActions());
