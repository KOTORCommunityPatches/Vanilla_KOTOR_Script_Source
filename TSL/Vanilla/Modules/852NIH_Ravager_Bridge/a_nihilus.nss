	...
	switch (nParam1) {
		case 0:
			...
			DelayCommand(6.0, AssignCommand(oDarthNihilus, ActionMoveToLocation(GetLocation(GetObjectByTag("sp_nihilus", 0)), 0)));
			{
				...
				DelayCommand(2.1, ApplyEffectToObject(2, effect1, oPC, 0.0));
				DelayCommand(2.1, ApplyEffectToObject(2, effect1, oVisasMarr, 0.0));
				DelayCommand(2.1, ApplyEffectToObject(2, effect1, oMand, 0.0));
		...
		case 14:
			...
			break;
		case 15:
			DelayCommand(3.4, SetGlobalFadeOut(0.1, 0.5, 0.0, 0.0, 0.0));
		...
		case 16:
			...
			DelayCommand(1.5, sub1());
			DelayCommand(1.6, CreatureFlourishWeapon(oPC));
			if ((GetGlobalNumber("852NIH_VisasSacrifice") != 1)) {
				AssignCommand(oVisasMarr, sub2(intGLOB_114, 0));
				AssignCommand(oVisasMarr, ClearAllEffects());
			}
			else {
				AssignCommand(oVisasMarr, sub2(intGLOB_114, 0));
				DelayCommand(3.1, AssignCommand(oVisasMarr, ActionPlayAnimation(26, 1.0, (-1.0))));
				AurPostString("Visas Is Dead...", 5, 5, 5.0);
			}
			AssignCommand(oMand, sub2(intGLOB_114, 0));
			AssignCommand(oDarthNihilus, sub2(intGLOB_114, 0));
