...
int StartingConditional() {
	int int1 = 0;
	if ((sub1(GetObjectByTag("VisasMarr", 0)) == 1)) {
		object oBodyItem = GetItemInSlot(1, GetObjectByTag("VisasMarr", 0));
		if ((GetTag(oBodyItem) == "a_robe_01")) {
			return 1;
		}
		else {
			string string2 = GetSubString(GetTag(oBodyItem), 0, 11);
			if ((string2 == "G_A_CLOTHES")) {
				return 1;
			}
		}
	}
