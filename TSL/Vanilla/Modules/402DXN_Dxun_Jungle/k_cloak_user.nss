...
void main() {
	...
	if ((int1 == 1001)) {
		...
		SetLocalBoolean(OBJECT_SELF, 21, 0);
		...
		while (GetIsObjectValid(oShapeObject)) {
			if ((GetStandardFaction(oShapeObject) == 1)) {
				if ((!GetLocalBoolean(OBJECT_SELF, 20))) {
					AurPostString("Enemy detected, cloaking", 5, 10, 5.0);
		...
		if ((GetLocalBoolean(OBJECT_SELF, 20) && (!GetLocalBoolean(OBJECT_SELF, 21)))) {
			AurPostString("No enemies nearby, decloaking", 5, 10, 5.0);
			SetLocalBoolean(OBJECT_SELF, 20, 0);
			RemoveEffectByID(OBJECT_SELF, 8000);
			effect effect5 = EffectVisualEffect(8001, 0);
			ApplyEffectToObject(1, effect5, OBJECT_SELF, 5.0);
		}
	}
	else {
		if ((int1 == 1002)) {
