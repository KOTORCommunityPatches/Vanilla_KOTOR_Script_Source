void main() {
	...
	AurPostString(("Stock: " + IntToString(GetGlobalNumber("501OND_Geeda_Stock"))), 6, 6, 2.0);
	if (((GetGlobalNumber("501OND_Geeda_Stock") + 1) == nGlobal)) {
		IncrementGlobalNumber("501OND_Geeda_Stock", 1);
		switch (nGlobal) {
			case 2:
		...
		if (((GetGlobalNumber("501OND_Geeda_Stock") + 2) == nGlobal)) {
			IncrementGlobalNumber("501OND_Geeda_Stock", 2);
			CreateItemOnObject("a_light_x08", GetObjectByTag("GeedaStore", 0), 1, 0);
			CreateItemOnObject("a_medium_07", GetObjectByTag("GeedaStore", 0), 1, 0);
			CreateItemOnObject("a_heavy_06", GetObjectByTag("GeedaStore", 0), 1, 0);
			CreateItemOnObject("a_robe_07", GetObjectByTag("GeedaStore", 0), 1, 0);
