...
void main() {
	if ((GetLocalBoolean(OBJECT_SELF, 58) == 0)) {
		...
		while ((int2 < 20)) {
			if ((int2 > 10)) {
				string1 = sub1(int2, 400);
			}
			else {
				string1 = sub1(int2, 0);
			}
			if ((int3 = FindSubString(string1, "[") >= 0)) {
				int4 = StringToInt(GetSubString(string1, (int3 + 1), 4));
				string2 = GetSubString(string1, 0, int3);
			}
			else {
				string2 = string1;
			}
			CreateItemOnObject(string2, GetObjectByTag("store_geg", 0), int4, 0);
			(int2++);
		}
	}
}
