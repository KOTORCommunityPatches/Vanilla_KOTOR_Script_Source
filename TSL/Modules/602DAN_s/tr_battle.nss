	...
	if (((GetGlobalNumber("601DAN_Side") && (!GetLocalBoolean(OBJECT_SELF, 50))) && (GetGlobalNumber("601DAN_Battle_Mili") >= 1))) {
		...
		if ((int6 == intGLOB_1)) {
			...
			if ((int6 == intGLOB_2)) {
				...
				if ((int6 == intGLOB_3)) {
					object object8 = GetObjectByTag("npc_azkul", 0);
					AssignCommand(object8, ClearAllActions());
					AssignCommand(oEntering, ClearAllActions());
					SetGlobalFadeIn(1.0, 1.0, 0.0, 0.0, 0.0);
					SetLocalBoolean(OBJECT_SELF, 50, 1);
					DelayCommand(1.0, AssignCommand(object8, ActionStartConversation(oEntering, "", 0, 0, 1, "", "", "", "", "", "", 0, 0xFFFFFFFF, 0xFFFFFFFF, 0)));
				}
				else {
					if ((int6 == intGLOB_4)) {
						object object10 = GetObjectByTag("npc_azkul", 0);
						AssignCommand(object10, ClearAllActions());
						AssignCommand(oEntering, ClearAllActions());
						SetGlobalFadeIn(1.0, 1.0, 0.0, 0.0, 0.0);
						SetLocalBoolean(OBJECT_SELF, 50, 1);
						DelayCommand(1.0, AssignCommand(object10, ActionStartConversation(oEntering, "", 0, 0, 1, "", "", "", "", "", "", 0, 0xFFFFFFFF, 0xFFFFFFFF, 0)));
					}
					else {
